...
public class PlainPermissionManager {
    ...
    public PlainAccessResource buildPlainAccessResource(PlainAccessConfig plainAccessConfig) throws AclException {
        ...
        List<RemoteAddressStrategy> allowedRemoteAddressStrategy = new ArrayList<>();
        List<String> allowedRemoteAddresses = plainAccessConfig.getAllowedRemoteAddresses();
        if (allowedRemoteAddresses != null && !allowedRemoteAddresses.isEmpty()) {
            for (String allowedRemoteAddress : allowedRemoteAddresses) {
                allowedRemoteAddressStrategy.add(remoteAddressStrategyFactory.getRemoteAddressStrategy(allowedRemoteAddress));
            }
        }
        plainAccessResource.setAllowedRemoteAddressStrategy(allowedRemoteAddressStrategy);
    ...
    public void validate(PlainAccessResource plainAccessResource) {
        ...
        // Check the allowed addr for accesskey
        List<RemoteAddressStrategy> allowedRemoteAddressStrategy = ownedAccess.getAllowedRemoteAddressStrategy();
        if (allowedRemoteAddressStrategy != null && !allowedRemoteAddressStrategy.isEmpty()) {
            boolean isAllowed = false;
            for (RemoteAddressStrategy remoteAddressStrategy : allowedRemoteAddressStrategy) {
                if (remoteAddressStrategy.match(plainAccessResource)) {
                    isAllowed = true;
                    break;
                }
            ...
            if (!isAllowed) {
                throw new AclException(String.format("Remote Address is not allowed for accessKey=%s", plainAccessResource.getAccessKey()));
            }
        }
